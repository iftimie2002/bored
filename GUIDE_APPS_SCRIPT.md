...
4. POST the ciphertext to the web app. Use a normal CORS JSON POST first so it shows up in `doPost`. If CORS fails, fall back to `sendBeacon` (it posts as `text/plain` without a preflight, but still reaches `doPost`):
