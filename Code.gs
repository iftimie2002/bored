...
function doPost(e) {
  return withErrorHandling(() => {
    ...
    // ADDED: capture geo from client IP without storing raw IP
    const clientIp = (e && e.context && e.context.clientIp) || '';
    payload.meta = payload.meta || {};
    payload.meta.geo = lookupGeo(clientIp);
...
function appendRow(payload) {
  ...
  // ADDED: derive anonymous client analytics
  const client = (payload.meta && payload.meta.client) || {};
  const uaInfo = deriveClientAnalytics(client.userAgent || '');
  const language = client.language || '';
  const timezone = client.timezone || '';
  const screenWidth = client.screen && client.screen.width ? client.screen.width : '';
  const screenHeight = client.screen && client.screen.height ? client.screen.height : '';
  const url = client.url || '';
  const referrer = client.referrer || '';
  const utm = client.utm || {};
  const country = (payload.meta && payload.meta.geo && payload.meta.geo.country) || '';
  const city = (payload.meta && payload.meta.geo && payload.meta.geo.city) || '';
  ...
  sheet.appendRow([
    ...
    // ADDED: anonymous analytics columns
    uaInfo.deviceType,
    uaInfo.osFamily,
